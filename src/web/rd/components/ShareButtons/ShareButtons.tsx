...
type ShareInfoProps = {
  ...
  trackType: Channel;
  url: string;
  Icon: typeof SocialTwitter | typeof SocialFacebook;
};

const ShareInfo = ({ trackType, onIconClick, url, Icon }: ShareInfoProps) => {

  const handleShareClick = useCallback(() => {
    onIconClick?.(trackType);
  }, [onIconClick, trackType]);

  return <a href={url} target="_blank" onClick={handleShareClick}>
    <Icon />
  </a>;
  ...
  return (
    <span className={classNames(styles.root, className)}>
      {shareInfo.map(({ Icon, url, trackType, name }) =>
        <ShareInfo key={name} Icon={Icon} url={url} trackType={trackType} onIconClick={onIconClick} />
      )}
    </span>
  );
};
