...
const RelatedContents: FC<RelatedContentsProps> = ({
  ...
  let rows = rowsToRender.map(({ contents: contentIds, id }) => {
  ...
  if (isNewDesign && !rowsToRender.length) {
    rows = [
      <RelatedContentsRow
        key={CONTAINER_ID_FOR_RELATED_RANKING}
        contentId={contentId}
        contentIds={[]}
        title={intl.formatMessage(containerTitles[CONTAINER_ID_FOR_RELATED_RANKING])}
        containerSlug={containerSlugs[CONTAINER_ID_FOR_RELATED_RANKING]}
        breakpoints={breakpoints}
        isVertical={isVertical}
        isNewDesign={isNewDesign}
        isPlaceholder
