...
const ContainerRow = ({ id, className, breakpoints, title, forceCurrentMode }: ContainerRowProps) => {
  ...
  const to = type === CONTAINER_TYPES.LINEAR ? getWebEpgUrl(id, { preferredLocale }) : getContainerUrl(id, { type, preferredLocale });
  const dispatch = useAppDispatch();
  const storeContainerPosition = useCallback(/* istanbul ignore next */({ index, containerId }: StoreContainerPositionParams) => {
    dispatch(setTileIndexInContainer({ index, containerId, isExplicit: false }));
  }, [dispatch]);
