...
import { toggleRemindLoginModal } from 'common/actions/ui';
...
import { messages as loginModalMessages } from 'web/components/LoginModal/loginModalMessages';
...
const useReminder = (options: UseReminderOptions) => {
  ...
  const addToReminder = useCallback(() => {
    if (isLoggedIn) {
      ...
      dispatch(
        toggleRemindLoginModal({
          isOpen: true,
          title: intl.formatMessage(loginModalMessages.remindLoginModalTitle),
          description: intl.formatMessage(messages.remindModalDesc, { contentTitle }),
          remindModalOnLogin: () => {
  ...
  }, [contentTitle, dispatch, dispatchAdd, isLoggedIn, intl]);
