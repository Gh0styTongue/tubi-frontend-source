...
import { loginModalSelector } from 'common/selectors/ui';
...
export const useTilePreviewPlayer = () => {
  const isPageVisible = useDocumentVisibility();
  const { isOpen: isLoginModalOpen } = useAppSelector(loginModalSelector);

  return {
    ...usePreviewPlayer(true),
    isPreviewPaused: !isPageVisible || isLoginModalOpen,
  };
};
