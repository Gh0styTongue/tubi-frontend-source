...
export const WebFoxLivePlayer = ({ contentId }: WebFoxLivePlayerProps) => {
  const { playerRef, fatalError, setFatalError, reload, sourceLoaded } = useFNGLive({ contentId });
  const { current: currentEvent } = usePurpleCarpet(contentId);
  const posterUrl = currentEvent?.landscapeImage;
