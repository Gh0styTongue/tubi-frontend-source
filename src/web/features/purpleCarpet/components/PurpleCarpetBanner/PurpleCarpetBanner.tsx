...
const ComingSoonBanner = ({ id, position }: { id: string; position: PurpleCarpeBannerPosition }) => {
  ...
  const logo = logoImage ? <img src={logoImage} className={styles.logo} alt="banner logo" /> : null;
  ...
  const rightLogo = !isLoggedIn && content.banner_images?.disclaimer_logo ? <img src={content.banner_images.disclaimer_logo} className={styles.rightLogo} alt="disclaimer logo" /> : null;
  const bg = isMobile
...
const DisasterModeBanner = ({ position }: { position: PurpleCarpeBannerPosition }) => {
  ...
  return (
    ...
    >
      <AlertFilled24 />
      <div className={styles.text}>
        {formatMessage(messages.disasterTip)}
      </div>
    </div>
  );
};
