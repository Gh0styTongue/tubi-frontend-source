...
const FeaturedRow = () => {
  ...
  const renderItem = useCallback((id: string, index: number) => (
    <MovieOrSeriesTile id={id} indexInContainer={index} tileOrientation={tileOrientation} showProgress hideMetadata={hideMetadata} rowIndexOffset={1} />
  ), [hideMetadata]);

  /* istanbul ignore next */
  const onIndexChange: CarouselProps<string>['onIndexChange'] = useCallback(({ itemIndex }: { itemIndex: number }) => {
