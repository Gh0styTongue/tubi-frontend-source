...
export const getPersonUrl = (personName: string) => {
  const name = encodePersonName(personName).replace(/^person-/, '');
  return getCanonicalLink(WEB_ROUTES.person, { id: getHashedName(name), name });
};

export const getPersonPath = (name: string) => {
  const url = getPersonUrl(name);
