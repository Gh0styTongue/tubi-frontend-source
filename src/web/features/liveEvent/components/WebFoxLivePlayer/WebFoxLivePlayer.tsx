...
import { useChromecastTabCasting } from 'common/features/liveEvent/playback/hooks/useChromecastTabCasting';
import useLiveEventLive from 'common/features/liveEvent/playback/hooks/useFoxLive';
...
export default function WebFoxLivePlayer({ contentId }: WebFoxLivePlayerProps) {
  const { playerRef, fatalError, setFatalError, reload, sourceLoaded, currentProgram } = useLiveEventLive({ contentId });
  ...
  const handleFatalErrorModalClose = useCallback((isRetry: boolean) => {
    if (isRetry) {
      /**
    ...
    setFatalError(false);
  }, [contentId, reload, retryCount, setFatalError]);
  return (
    <>
      <div
      ...
      <WebErrorModal
        ...
        playerErrorDetails={PLAYER_ERROR_DETAILS.LIVE_PLAYBACK_ERROR}
      />
    </>
  );
}
