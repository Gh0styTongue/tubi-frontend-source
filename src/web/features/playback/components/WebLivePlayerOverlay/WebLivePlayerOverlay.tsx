...
import { useLiveVolumeHandler } from './hooks/useLiveVolumeHandler';
...
export interface WebLivePlayerOverlayProps {
  ...
  lang?: string;
  // control visual visibility while keeping DOM mounted
  isVisible?: boolean;
}
...
> = ({
  ...
  const { destroyPlayers, isFloating } = usePlayerPortal();
  ...
  const showPipButton = isPictureInPictureSupported() && !isFloating;
  ...
  const { toggleMute, setVolume, volume, isMuted, showVolumeBar, hideVolumeBar } = useLiveVolumeHandler({
    wrapper,
    volumeExpand,
    setVolumeExpand,
  ...
  const onCloseFloatingPlayer = useCallback(() => {
    destroyPlayers();
  }, [destroyPlayers]);
  ...
  const onScaleUpClick = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    const titleUrl = getUrlByVideo({ video, preferredLocale });
  ...
  }, [video, programIndex, contentIndex, contentId, containerId, preferredLocale]);
  ...
  const onTvGuideClick = useCallback(() => {
    onExitFullscreen();
    tubiHistory.push(addLocalePrefix(preferredLocale, WEB_ROUTES.live));
  }, [onExitFullscreen, preferredLocale]);
  ...
  let volumeTooltip: string | undefined;
  if (isFloating) {
    if (isMuted) {
      volumeTooltip = intl.formatMessage(messages.unmute);
    } else {
      volumeTooltip = intl.formatMessage(messages.mute);
    }
  }
  ...
  return (
    <React.Fragment>
      ...
      <div
        className={
          classnames(styles.livePlayerArea, { [styles.livePlayerAreaHidden]: !isVisible })}
        ref={playerAreaRef}
      >
        {children}
        <div
        ...
        >
          ...
          <div className={controlAreaClassnames}>
            ...
            <div className={styles.controlsLeft}>
              {isFloating || (useMobileDesign && fullscreen)
            ...
            <div className={isEpgPlayerView ? styles.epgPlayerControlsWrapper : styles.controlsWrapper}>
              { castButtonAvailable && (!useMobileDesign && !isFloating ? <IconButton
              ...
              <Volume
                ...
                show={!isMobile && !isMuted && !!volumeExpand && !isFloating}
                isLive
                isMiniPlayer={isEpgPlayerView}
                tooltip={volumeTooltip}
                tooltipPlacement="bottom"
              />
              {hasSubtitle && !isFloating ? (
                <ClosedCaptionsControl {...captionsProps} />
              ) : null}
              {showPipButton ? (
                <IconButton
                  data-test-id="pictureInPictureButton"
                  icon={pipEnabled ? <PictureInPictureClose24 className={controlItemClass} /> : <PictureInPictureOpen24 className={controlItemClass} />}
            ...
            {isFloating ? (
              <IconButton
                containerClassName={styles.closeFloatingPlayerButton}
                icon={<CloseStroke />}
                onClick={onCloseFloatingPlayer}
