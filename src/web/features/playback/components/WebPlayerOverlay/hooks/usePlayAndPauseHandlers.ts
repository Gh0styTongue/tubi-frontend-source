...
export const usePlayAndPauseHandlers = ({
  ...
  const { getPlayerDisplayMode } = usePlayerPortal();
    ...
    (explicit: boolean) => {
      ...
      return dispatch(controlActions.play(explicit ? ActionLevel.UI : ActionLevel.CODE))
        .then(() => {
          ...
          if (!explicit) return;
          refreshActiveTimer();
          const pauseToggleEventObject = buildPauseToggleEventObject(contentId, PauseState.RESUMED, getPlayerDisplayMode());
  ...
  const pause = useCallback((explicit: boolean) => {
    return dispatch(controlActions.pause()).then(() => {
      if (!explicit) return;
      refreshActiveTimer();
      const pauseToggleEventObject = buildPauseToggleEventObject(contentId, PauseState.PAUSED, getPlayerDisplayMode());
      trackEvent(eventTypes.PAUSE_TOGGLE, pauseToggleEventObject);
    });
  }, [
    dispatch,
