...
const MIN_BROWSER_VERSION_FOR_CDM = 133;
...
const WebErrorModal: React.FunctionComponent<WebErrorModalProps> = ({ isOpen, onClose, playerErrorDetails }) => {
  ...
  const transitionOverlayRef = useRef(false);
  const transitionDialogRef = useRef(false);
  const [isTransitionIn, setTransitionIn] = useState(false);

  const handleTransitionEnterOverlay = useCallback(() => {
    transitionOverlayRef.current = true;
  }, []);

  const handleTransitionEnterDialog = useCallback(() => {
    transitionDialogRef.current = true;
  }, []);

  const tryToExit = useCallback(() => {
    if (transitionDialogRef.current || transitionOverlayRef.current || !onClose) {
      return;
    }
    onClose(retryRef.current);
  }, [onClose]);

  const handleTransitionExitedOverlay = useCallback(() => {
    transitionOverlayRef.current = false;
    tryToExit();
  }, [tryToExit]);

  const handleTransitionExitedDialog = useCallback(() => {
    transitionDialogRef.current = false;
    tryToExit();
  }, [tryToExit]);
    ...
    if (playerErrorDetails === PLAYER_ERROR_DETAILS.AD_REQUEST_BLOCKED_ERROR) {
      return (<section className={styles.webErrorModalContentBodyDescription}>
        ...
        <ul>
          {
            [2, 3].map(index => (
              <li key={index}>{intl.formatMessage(messages[`errorAdRequestBlock${index}` as keyof typeof messages])}</li>
            ))
          }
        </ul>
      ...
      <ThemeProvider>
        <div className={styles.webErrorModalRoot}>
          <CSSTransition
            ...
            in={isTransitionIn}
            onEnter={handleTransitionEnterOverlay}
            onExited={handleTransitionExitedOverlay}
            timeout={300}
            nodeRef={overlayRef}
          >
          ...
          <CSSTransition
            ...
            in={isTransitionIn}
            onEnter={handleTransitionEnterDialog}
            onExited={handleTransitionExitedDialog}
            timeout={300}
            nodeRef={dialogRef}
          >
