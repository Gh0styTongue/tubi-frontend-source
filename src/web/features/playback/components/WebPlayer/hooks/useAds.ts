...
import { isEspanolModeEnabledSelector, isKidsModeEnabledSelector, osNameSelector } from 'common/selectors/ui';
...
import useRequestAdsAfterSeek from 'ott/features/playback/components/OTTPlaybackController/hooks/useRequestAdsAfterSeek';
...
export const useAds = ({ adBreaks, isFromAutoplay, video, isDeeplink, showAutoPlay }: UseAdsParams) => {
  ...
  const playAdResponse = useCallback((response: AdPod) => {
    ...
    player?.playAdResponse(response);
  }, [getPlayerInstance, contentIdRef, showAutoPlayRef]);

  useRequestAdsAfterSeek({
    adBreaks,
    getAdUrl: getContentAdUrl,
    playAdResponse,
    contentId: video.id,
  });
  ...
  const onTime = useCallback(({ position }: { position: number }) => {
    ...
    if (adSchedule.hasOwnProperty(nowPos) && !adSchedule[nowPos].adPlayed) {
      ...
      const isMockAdUrl = !FeatureSwitchManager.isDefault(['Ad', 'MockUrl']);
      VODPlaybackSession.getInstance().adPodFetch({
        isPreroll: false,
      });
      fetchAds(adUrl, { isMock: isMockAdUrl }).then(({ ads, metrics }) => {
        VODPlaybackSession.getInstance().adPodFetchSuccess({
          isPreroll: false,
          responseTime: metrics.responseTime,
          adsCount: ads && ads.length,
        });
        playAdResponse(ads);
      }).catch((error) => {
  ...
  }, [getPlayerInstance, playAdResponse]);
