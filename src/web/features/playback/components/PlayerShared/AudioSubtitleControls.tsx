...
const AudioSubtitleControls: React.FC<AudioSubtitleControlsProps> = (props: AudioSubtitleControlsProps) => {
  ...
  const hideCaptionsList = useCallback(() => {
    // Don't hide if advanced drawer is open
    if (isAdvancedDrawerOpen) return;
  ...
  }, [visible, onToggle, toggleDelay, isAdvancedDrawerOpen]);

  const handleCaptionLanguageChange = useCallback((idx: number) => {
    setCaptions(idx !== captionsIndex ? idx : 0);
  }, [captionsIndex, setCaptions]);

  const handleAdvancedDrawerStateChange = useCallback((isOpen: boolean) => {
    setIsAdvancedDrawerOpen(isOpen);
    onAdvancedDrawerStateChange?.(isOpen);
  }, [onAdvancedDrawerStateChange]);

  const handleClickOrEnderCaptionsIcon = useCallback((isFromEnter: boolean, e: React.MouseEvent|React.KeyboardEvent) => {
    e.stopPropagation();
  ...
  const handleClickCaptionsIcon = useCallback((e: React.MouseEvent|React.KeyboardEvent) => {
    handleClickOrEnderCaptionsIcon(false, e);
  }, [handleClickOrEnderCaptionsIcon]);

  const handleEnterCaptionsIcon = useCallback((e: React.MouseEvent|React.KeyboardEvent) => {
    handleClickOrEnderCaptionsIcon(true, e);
  }, [handleClickOrEnderCaptionsIcon]);
  ...
  return (
    ...
    >
      <IconButton
        ...
        onClick={handleClickCaptionsIcon}
        onEnter={handleEnterCaptionsIcon}
        className={styles.cc}
        containerClassName={classNames({ [styles.popupShow]: visible })}
      />
      {visible ? <AudioSubtitleMenu {...props} handleCaptionLanguageChange={handleCaptionLanguageChange} onAdvancedDrawerStateChange={handleAdvancedDrawerStateChange} /> : null}
    </span>
  );
};
