...
import ActivationSuccess from 'web/features/authentication/components/ActivationSuccess/ActivationSuccess';
...
interface ActivationCodeFormProps extends WithRouterProps<any, any> {
  dispatch: TubiThunkDispatch;
  prePopulatedActivationCode?: string | string[];
  onSubmit?: (code: string, formikBag: FormikBag<{ dispatch: TubiThunkDispatch }, FormValues>) => void,
  waitForAutoActivate?: boolean;
  showSuccessDirectly?: boolean;
}

export const ActivationCodeForm: React.FC<FormikProps<FormValues> & ActivationCodeFormProps> = ({
  isSubmitting,
  handleSubmit,
  ...
  useEffect(() => {
    ...
    if (error) {
      setFieldTouched('activationCode', true);
      setFieldError('activationCode', error.message);
    }
  }, [location.state?.error, setFieldError, setFieldTouched]);
  ...
  useEffect(() => {
    // preload kids on activation success
    if (success) {
...
const handleSubmit = (
  ...
  if (onSubmit) {
    onSubmit(activationCode, formikBag);
  } else {
...
export const withFormikConfig: WithFormikConfig<ActivationCodeFormProps, FormValues> = {
  mapPropsToValues: props => ({ activationCode: props.prePopulatedActivationCode as string || '' }),
  mapPropsToStatus: (props) => {
    if (props.location.state?.error) {
      return { success: false };
    }
    return { success: props.showSuccessDirectly || undefined };
  },
...
export default withRouter(withFormik(withFormikConfig)(ActivationCodeForm));
