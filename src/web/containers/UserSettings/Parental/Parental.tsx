...
export class Parental extends PureComponent<ParentalProps, ParentalState> {
  ...
  handleSave = () => {
    ...
    const { dispatch, location, destroyPlayers, parentalRating: currentParentalRating } = this.props;

    this.setState({ updateParentalError: '' });

    if (!unsaved) return Promise.reject();

    return dispatch(updateParental({ location, rating: parentalRating, password }))
      .then(() => {
        this.showNotification(parentalRating);
        this.setState({ unsaved: false, password: '', updateParentalError: '' });
        // Destroy any floating player if the new rating is lower than the current rating
        if (parentalRating < currentParentalRating) {
          destroyPlayers();
        }
      })
      .catch((error: AuthError) => {
        this.setState({ updateParentalError: error.message });
  ...
  private handleSelectChange = ({ value }: DropdownOption) => this.onSelectChange({ target: { value } });
  ...
  render() {
    ...
    return (
      <div className={sharedStyles.main} data-test-id="parental-refresh">
        ...
        {hasPassword ? (
          <React.Fragment>
            <p id="parental-rating-description" className={sharedStyles.subheader}>
            ...
            <form className={styles.formContainer}>
              ...
              <fieldset>
                ...
                <Dropdown
                  ...
                  aria-describedby="parental-rating-description"
                  aria-label={`${formatMessage(messages.currentRating)}`}
                />
                <p id="password-description" className={styles.passwordDescription}>{formatMessage(messages.enterPw)}</p>
                <TextInput
                  canShowPassword
                  name="password"
...
const ConnectedParental = connect(mapStateToProps)(injectIntl(Parental));

const FunctionalParentalContainer: React.FC<Omit<ParentalProps, 'destroyPlayers'>> = (props) => {
  const { destroyPlayers } = usePlayerPortal();
  return <ConnectedParental {...props} destroyPlayers={destroyPlayers} />;
};

export default FunctionalParentalContainer;
