...
import { setRenderedContainersCount } from 'common/actions/ui';
...
type StateProps = {
  ...
  viewportType: string;
  preferredLocale: LocaleOptionType | undefined;
};
...
export class Containers extends Component<Props> {
  ...
  render() {
    const { containerInHome, currentContentMode, isHomePage, containers, isMyStuffPage, renderedContainersCount, containerIndexMap, isWebEpgEnabled, containersListFullyLoaded, intl, personalizationId, preferredLocale } = this.props;
    ...
    let containersClassName = '';
    if (containerInHome) {
      containersClassName = styles.containerInHome;
    }
    return (
      <div ref={this.assignRef} className={containersClassName} data-test-id="web-container-component">
            ...
            (<React.Fragment><TransitionGroup component="div" exit={false}>
              {
                containerShowingRows.map((container, idx) => {
                  ...
                  const to = type === CONTAINER_TYPES.LINEAR && isWebEpgEnabled
                    ? getWebEpgUrl(id)
                    : addLocalePrefix(preferredLocale, getContainerUrl(id, { type }));
...
export const mapStateToProps = (state: StoreState, props: OwnProps & Pick<WithRouterProps, 'location'>) => {
  ...
  const {
    ui: { renderedContainersCount, containerIndexMap, viewportType, preferredLocale },
    auth: { firstSeen },
  } = state;
  // if the user is on the my stuff page, we need to use the 'all' content mode for the menu list
  const myStuffForceCurrentMode = contentModeForMenuListSelector(state, { pathname });
  const currentContentMode = contentMode || currentContentModeSelector(state, { pathname });
  const container = containerSelector(state, { forceCurrentMode: isMyStuffPage ? myStuffForceCurrentMode : contentMode, pathname });
  ...
  const isHomePage = pathname === WEB_ROUTES.home;
  ...
  return {
    ...
    containers: isMyStuffPage
      ? webMyStuffContainerSelector(state, {
        forceCurrentMode: myStuffForceCurrentMode,
        pathname,
      })
