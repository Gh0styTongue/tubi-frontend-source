...
export const setup = (player: LivePlayerWrapper | Player, options: SetupOptions): Monitoring => {
  ...
  // If no experiment is running, we don't need to wait for param7
  const pendingMetadata = ['cdn', runningExperiments ? 'param7' : undefined].filter(Boolean);
  ...
  const setupOptions: Record<string, unknown> = {
    ...
    // https://bitbucket.org/npaw/lib-plugin-js/src/6e7a0c553797dd3634e475ac825ef82b3f192dc5/src/plugin/options.js#lines-620
  ...
  return {
    ...
    setOptions: (key: string, value: unknown) => {
      ...
      plugin.setOptions(setupOptions);
    },
    getOptions: () => setupOptions,
  };
};
