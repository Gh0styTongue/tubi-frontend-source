...
import {
  ...
  PLAYER_QOS_METRIC_DISTRIBUTION_TYPE_KEYS,
  PLAYER_QOS_METRIC_INCREMENT_TYPE_KEYS,
  PLAYER_LIVE_QOS_METRIC_INCREMENT_TYPE_KEYS,
  PLAYER_LIVE_QOS_METRIC_DISTRIBUTION_TYPE_KEYS,
...
export const trackPlayerQosMetrics = (metrics: {
  ...
  const incrementMetrics: { [key: string]: number } = {};
  const distributionMetrics: { [key: string]: number } = {};
...
export const trackLivePlayerQosMetrics = (metrics: {
  [x: string]: number;
}, tags?: {
  [x: string]: string;
}) => {
  ...
  const distributionMetrics: { [key: string]: number } = {};
  Object.keys(metrics).forEach(key => {
    if (PLAYER_LIVE_QOS_METRIC_INCREMENT_TYPE_KEYS.includes(key)) {
      incrementMetrics[`liveQos.${key}`] = metrics[key];
    } else if (PLAYER_LIVE_QOS_METRIC_DISTRIBUTION_TYPE_KEYS.includes(key)) {
      distributionMetrics[`liveQos.${key}`] = metrics[key];
    }
  });
  if (Object.keys(incrementMetrics).length > 0) {
    trackPerformanceMetrics(incrementMetrics, tags, 'increment');
