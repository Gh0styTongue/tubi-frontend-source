...
export type PlayerAnalyticsEventLiveAdStart = {
  track_id: string;
  video_id?: string;
  ad_id: string;
  url: string;
...
export function playerAnalyticsLiveAdStart(data: PlayerAnalyticsEventLiveAdStart) {
  trackAnalyticsIngestionV3({
    name: PLAYER_ANALYTICS_EVENT_NAME.LIVE_AD_START,
    message: data,
  });
}
