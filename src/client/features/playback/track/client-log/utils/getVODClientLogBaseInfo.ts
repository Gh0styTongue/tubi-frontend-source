...
export function getVODClientLogBaseInfo({ contentId, videoResource, playerInstance, ...rest }: VODClientLogBaseParams): VODClientLogBaseInfo {
  ...
  const rawSrcUrl = playerInstance?.getCurrentVideoElement()?.src;
  const srcUrl = rawSrcUrl ? trimQueryString(rawSrcUrl) : rawSrcUrl;
  return {
    ...rest,
    ...
    contentBwEstimate: contentBwEstimate !== undefined ? toFixed2(contentBwEstimate / 1000) : -1, // kbps
