import type { PlatformUppercase } from 'common/types/platforms';
...
export function platformSamplerFactory(
  ...
  // Shared sampling decision for the lifetime of the sampler
  let sharedAllowDecision: boolean | undefined;
  ...
  function platformSampler<F extends Function>(fnToSample: F): F | ((...args: any[]) => void) {
    ...
    // Lazily compute the sampling decision once
    if (sharedAllowDecision === undefined) {
      const diceRoll = Math.random() * 100;
      sharedAllowDecision = diceRoll < resolvedConfig.percentageToAllow;
    }

    return sharedAllowDecision ? fnToSample : () => {};
  }
