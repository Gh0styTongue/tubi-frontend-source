...
import type { FeatureInfo } from 'common/playback/VODPlaybackSession';
...
/**
 * Base interface for Live Player Analytics data
 * Contains common fields used across different live analytics events
 */
export interface LivePlayerAnalyticsBaseInfo {
  track_id: string;
  video_id?: string;
...
export interface LiveErrorClientLogInfo extends ErrorClientLogInfo {
  cdnCache?: string;
  cdnProvider?: string;
  cdnServedBy?: string;
  cdnCloudFrontId?: string;
  cdnCloudFrontPop?: string;
}
...
interface VideoErrorParamsBase {
  isAd?: boolean;
  videoUrl?: string;
  position?: number;
  contentId?: string;
...
export interface PauseAdBaseLogContext {
  ...
  // type of ad shown, defaults to 'image' if not specified
  adType: 'image' | 'video',
  // API version used for the pause ad request
  apiVersion: 'v1' | 'v2',
  // log version for tracking schema changes
  logVersion: number,
}
