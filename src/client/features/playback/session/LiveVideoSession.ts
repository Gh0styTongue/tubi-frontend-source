...
import type { ErrorEventData, QualityLevel } from '@adrise/player';
...
export type LiveStage = LivePlaybackStage
  ...
  | 'PAUSED'
  // IN_APP_PICTURE_IN_PICTURE: The player is playing in the in-app picture-in-picture mode.
  | 'IN_APP_PICTURE_IN_PICTURE';
...
export type VideoQualityFeedback = 'good' | 'bad' | 'initial';
...
export interface LiveVideoSession {
  ...
  qualityFeedback: VideoQualityFeedback;
  qualityLevelOnFeedback?: QualityLevel;
  qualityVideoHeightOnFeedback?: number;
  qualityToastShown: boolean;
...
const initialVideoSession: LiveVideoSession = {
  ...
  isRevisit: false,
  isDeliberate: false,
  qualityFeedback: 'initial',
  qualityLevelOnFeedback: undefined,
  qualityVideoHeightOnFeedback: -1,
...
function setPlaybackStage() {
  const {
    ...
    withEPG, isPaused, playerDisplayMode,
  } = videoSession;
  const isInAppPip = playerDisplayMode === PlayerDisplayMode.IN_APP_PICTURE_IN_PICTURE;
  if (isPaused) {
  ...
  if (isInAppPip) {
    videoSession.stage = 'IN_APP_PICTURE_IN_PICTURE';
    return;
  }
  if (isPreview) {
...
function updateDuration(position: number) {
  ...
  const { prevPosition, isPreview, withEPG, isFullscreen, playerDisplayMode } = videoSession;
  videoSession.prevPosition = positionInt;
  const isInAppPip = playerDisplayMode === PlayerDisplayMode.IN_APP_PICTURE_IN_PICTURE;
  if (prevPosition < 0 || positionInt === prevPosition) return;
  if (isInAppPip) {
    videoSession.inAppPipDuration++;
  }
  if (isPreview) {
...
export function toggleFullscreen(state: PlayerDisplayMode) {
  if (state === PlayerDisplayMode.IN_APP_PICTURE_IN_PICTURE) {
...
export function updateQualityFeedback(feedback: VideoQualityFeedback, qualityLevel?: QualityLevel) {
  videoSession.qualityFeedback = feedback;
  videoSession.qualityLevelOnFeedback = qualityLevel;
  videoSession.qualityVideoHeightOnFeedback = qualityLevel?.height;
}

export function markQualityToastShown() {
  videoSession.qualityToastShown = true;
}

export function setPlayerDisplayMode(playerDisplayMode: PlayerDisplayMode) {
  videoSession.playerDisplayMode = playerDisplayMode;
}
