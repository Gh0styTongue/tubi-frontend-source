...
import { isFireTVDevice, isFireTVGen1, isFireTVStickGen1, isHisense5659, isHisense6886, isChromeVersionBelow68OnLGTV } from 'client/utils/clientTools';
import { DRM_ROBUSTNESS_RULE } from 'common/constants/player';
import { getExperiment } from 'common/experimentV2';
import webottLgtvDisableWebWorker from 'common/experimentV2/configs/webottLgtvDisableWebWorker';
import FeatureSwitchManager from 'common/services/FeatureSwitchManager';
import type { HDCPVersion } from 'common/types/video';
import { isSamsung2017 } from 'common/utils/tizenTools';
...
export function getHlsPlatformSpecificProps({
  ...
  forceClearMediaKeysAfterDetached = false,
}: {
  emeEnabled?: boolean,
  ...
  let platformSpecificProps: Partial<HlsConfig> = {
  ...
  switch (__OTTPLATFORM__) {
     ...
     */
    case 'LGTV': {
      const disableWebWorker = getExperiment(webottLgtvDisableWebWorker, { disableExposureLog: true }).get('disable_web_worker_v0');
      platformSpecificProps = {
        ...platformSpecificProps,
        enableWorker: meetsLgtvWorkerVersionRequirement() && !disableWebWorker,
        stallMinimumDurationMS: isChromeVersionBelow68OnLGTV() ? 1000 : 550,
      };
      break;
    }
    case 'FIRETV_HYB':
    ...
    case 'XBOXONE':
      ...
      platformSpecificProps = {
        ...
        enableWorker: true,
        enableResolveWithMediaKeyStatusPending: true,
      };
      break;
    case 'COMCAST':
      platformSpecificProps = {
    ...
    case 'PS4':
      platformSpecificProps = {
        ...
        forceFilterDrmKeySystemWithConfig: !deprecateFilterDrmKeySystemWithConfig,
        enableResolveWithMediaKeyStatusPending: deprecateFilterDrmKeySystemWithConfig,
        highBufferWatchdogPeriod: 8,
    ...
    case 'TIZEN':
      platformSpecificProps = {
        ...
        highBufferWatchdogPeriod: 8,
        disableMediaKeySessionRemove: true,
        nudgeMaxRetry: 0,

      };
      if (isSamsung2017()) {
        platformSpecificProps.enableSkipKeyStatusChangeListener = true;
      }
      break;
    case 'TIVO':
...
export function meetsLgtvWorkerVersionRequirement(): boolean {
  if (__OTTPLATFORM__ !== 'LGTV') {
    return false;
  }
  const systemVersion = systemApi.getSystemVersion?.();
  return !!systemVersion && isValidSemver(systemVersion) && semverCompareTo(systemVersion, '3.9.0') > 0;
}
...
// note- this is only used by NativePlayer. Samsung, Hulu, Web use their own
