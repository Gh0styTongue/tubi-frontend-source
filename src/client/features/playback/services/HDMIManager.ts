...
import { PauseState, PlayerDisplayMode } from '@tubitv/analytics/lib/playerEvent';
...
const sourceMap: Record<PlayerName.Linear | PlayerName.Preview, PlayerSource> = {
...
export function getHDMIManagerConfig(player: Player | LivePlayerWrapper, contentId: string): ConstructorParameters<typeof HDMIManager>[0] {
  if (player.playerName === PlayerName.Linear || player.playerName === PlayerName.Preview) {
    return {
      player,
...
export class HDMIManager extends BasePlayerManager<Player | LivePlayerWrapper> {
  ...
  private onHDMIConnected = ({ hdmi_connection }: { hdmi_connection: boolean }) => {
    ...
    if (this.player.playerName === PlayerName.VOD) {
      VODPlaybackSession.getInstance().setHdmiConnection(hdmi_connection);
    }
  ...
  private trackPauseToggleEvent(pauseState: PauseState) {
    const pauseToggleEventObject = buildPauseToggleEventObject(this.contentId, pauseState, PlayerDisplayMode.DEFAULT);
