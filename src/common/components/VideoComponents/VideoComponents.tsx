...
export const Duration = ({ duration = 0, className }: { duration?: number; className?: string }) => {
  const intl = useIntl();
  const humanReadDuration = secondsToHoursAndMinutes(duration, { formatMessage: intl.formatMessage });
...
export const RatingWithDescriptor = ({
  ...
  return (
    <div className={classNames(styles.ratingWithDescriptor)}>
      ...
      {shouldShowRatingOverlayFullHDBadge ? <FullHD cls={styles.fullHDBadge} /> : null}
