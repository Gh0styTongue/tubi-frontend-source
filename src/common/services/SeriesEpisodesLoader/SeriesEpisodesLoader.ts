...
export default class SeriesEpisodesLoader {
  ...
  private checkIfMetadataLoaded(): boolean {
    ...
    const content = this.storeLike?.getState().video.byId[this.seriesContentId];
    const metadataFromStore = content?.seasons;
    const num_seasons = content?.num_seasons;
    const isMetadataLoaded = content?.isMetadataLoaded;
    if (metadataFromStore && (isMetadataLoaded || metadataFromStore?.length === num_seasons)) {
