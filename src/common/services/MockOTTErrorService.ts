...
const triggerMockError = (key: string) => {
  try {
    if (mockHLSError[key as keyof typeof mockHLSError]) {
      window.Tubi?.player?.adapter.extension?.hls.trigger('hlsError' as any, mockHLSError[key as keyof typeof mockHLSError]);
    ...
    } else if (mockPlayerError[key as keyof typeof mockPlayerError]) {
      window.Tubi?.player?.adapter.videoElement.pause();
      window.Tubi?.player?.adapter.emit('tb_error' as keyof PlayerListeners, mockPlayerError[key as keyof typeof mockPlayerError]);
    } else {
...
const MockOTTErrorService = {
  ...
  getAllErrorTypes() {
    const allErrorTypes: Record<string, string> = {};
