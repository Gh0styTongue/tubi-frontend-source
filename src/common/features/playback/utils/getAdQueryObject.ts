...
export const getAdQueryObject = ({
  ...
  let gdprQuery: Record<string, unknown> = {};
  if (addGDPRQuery) {
    const rainmakerParams = OnetrustClient.getRainmakerParams();
    gdprQuery = { ...rainmakerParams.query };
    if (includeTCFQuery) {
      const tcfString = rainmakerParams.header?.['X-Tubi-TCF-String'];
      if (tcfString) {
        gdprQuery.tcf = tcfString;
      }
    }
  }
