...
import { trackInvalidCDNResource } from 'client/features/playback/track/client-log';
...
import {
  ...
  RESOURCE_LEVEL_LOCAL_STORAGE_KEY,
} from 'common/constants/constants';
import { hlsNotEnabledVersions } from 'common/selectors/tizen';
import FeatureSwitchManager from 'common/services/FeatureSwitchManager';
import { MockVideoResourceConstant } from 'common/services/MockVideoResourceConstant';
...
import { setLocalData, getLocalData, removeLocalData } from 'common/utils/localDataStorage';
...
const getVideoResourceManager = (options: VideoResourceManagerOptions): VideoResourceManager => {
  ...
  const VODFeatureSwitchValue = FeatureSwitchManager.get(['VOD', 'MockVODVideoResource']) as keyof typeof MockVideoResourceConstant | undefined;
  if ((!__PRODUCTION__ || __IS_ALPHA_ENV__) && VODFeatureSwitchValue && Array.isArray(MockVideoResourceConstant[VODFeatureSwitchValue])) {
    options.videoResources = MockVideoResourceConstant[VODFeatureSwitchValue] as VideoResource[];
  }
...
export default class VideoResourceManager {
  ...
  private videoResourceLevelsForCurrentCodec: VideoResourceLevel[];
  ...
  constructor(options: VideoResourceManagerOptions) {
    this.filterOutUnsupportedResources(options);
    this.videoResourceCodecLevels = this.createLevels(options);
    this.videoResourceLevelsForCurrentCodec = this.videoResourceCodecLevels[this.codecIndex]?.resources ?? [];
    ...
    if (__CLIENT__) {
      exposeToTubiGlobal({ videoResourceManager: this });
    }
  }
   ...
   * 3. Platform: On platforms we don't support HEVC, including TIVO
   */
  checkCodecFallback() {
    /* istanbul ignore next */
    ...
    if (!__IS_HEVC_720P_SUPPORTED__) {
      ...
      // But new Tivo will support HEVC we don't need to codec fallback
      if (__OTTPLATFORM__ === 'TIVO' && isNewTivoOSVersion()) {
        return;
      }
      this.codecFallback();
    }
  ...
  codecFallback(drmIndex?: number): VideoResource | undefined {
    ...
    this.codecIndex++;
    this.DRMIndex = 0;
    this.videoResourceLevelsForCurrentCodec = fallbackVideoResourceLevels;
  ...
  getAllLevelsForCurrentCodec(): VideoResourceLevel[] {
    return this.videoResourceLevelsForCurrentCodec;
  }

  getAllVideoResources(): VideoResource[] {
    return this.videoResourceCodecLevels.flatMap((codecLevel) => codecLevel.resources.flatMap((resource) => resource.resources));
  }
    ...
    for (let i = this.codecIndex; i < this.videoResourceCodecLevels.length; i++) {
      ...
      for (let j = 0; j < resources.length; j++) {
        if (resources[j].drm === defaultLevel.drm
          ...
          this.videoResourceLevelsForCurrentCodec = resources;
          return;
        }
      }
    }
  ...
  getCurrentDRMResource(drmIndex?:number): VideoResourceLevel | undefined {
    ...
    const currentResourceLevel = this.videoResourceLevelsForCurrentCodec[drmIndex];
  ...
  private checkIfUseHlsOnSamsung() {
    try {
  ...
  private filterOutUnsupportedResources(options: VideoResourceManagerOptions) {
    if ((__OTTPLATFORM__ === 'TIZEN' && this.checkIfUseHlsOnSamsung()) // Samsung Hls.js
      || __OTTPLATFORM__ === 'PS4') { // PS4 WebMAF3 Hls.js
      options.videoResources = options.videoResources.filter(item => item.type.includes('hls'));
    } else if (__OTTPLATFORM__ === 'TIZEN') { // Samsung AVPlayer
      options.videoResources = options.videoResources.filter(item => item.type.includes('dash') || item.type === 'hlsv3');
    }
  }
   ...
   * - DRM w/ HDCP / Resolution
   * - DRM w/o HDCP / Resolution
   * - non-DRM
   */
  private groupVideoResourceByDRM(options: VideoResourceManagerOptions): VideoResourceLevel[] {
    ...
    for (const cur of filteredWithDRM) {
      const key = `${cur.drm}${cur.hdcp}${cur.resource.resolution}`;
