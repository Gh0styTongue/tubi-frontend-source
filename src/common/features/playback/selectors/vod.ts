...
import { WRAPPER_TAG } from 'common/features/wrapper/constants';
...
import type { AdBreaks, Video } from 'common/types/video';
...
export const contentSelector = createSelector(
  ...
  (video, activePreviewVideoId, containerChildrenIdMap, activeContainerId, gridIndex, recommendations, search, ownProps) => {
    ...
    const isContainerListPage = currentPage === BgPageType.CONTAINER_LIST;

    if (![BgPageType.HOME, BgPageType.MY_STUFF, BgPageType.DETAILS, BgPageType.CONTAINER_DETAILS, BgPageType.CONTAINER_LIST].includes(currentPage)) {
    ...
    if ([BgPageType.CONTAINER_DETAILS].includes(currentPage) || isContainerListPage) {
      const { byId } = video;
...
export const isWrapperContentSelector = createSelector(
  contentSelector,
  (content) => content.tags?.includes(WRAPPER_TAG),
);

export const isLiveEventContentSelector = createSelector(
  contentSelector,
  (content) => isLiveEvent(content),
);
