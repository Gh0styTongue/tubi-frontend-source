...
export const loadFavoritedChannelIds = (mode: LiveContentMode): EPGThunk => (
  ...
  return dispatch({
    ...
    mode,
    payload: () => fetchWithToken(`${apiConfig.accountServiceUserPrefix}/preferences/rate`, {
      params: {
        type: 'liked',
        target: 'linear',
      },
...
const createToggleFavoriteChannelAction = (
  client: ApiClient,
  dispatch: TubiThunkDispatch<EPGActionRF>,
  getState: () => StoreState,
  contentIds: string[],
  ...
  return {
    ...
    payload: () => dispatch(fetchWithToken(`${apiConfig.accountServiceUserPrefix}/preferences/rate`, {
      method: 'patch',
      data: {
        target: 'linear',
        action,
...
export const addFavoriteChannel = (contentId: string, mode: LiveContentMode = LIVE_CONTENT_MODES.all): EPGThunk =>
  async (dispatch, _getState, client) => {
    await dispatch(createToggleFavoriteChannelAction(client, dispatch, _getState, [contentId], 'like', mode));
  };

export const batchFavoriteChannel = (contentIds: string[], mode: LiveContentMode = LIVE_CONTENT_MODES.all): EPGThunk =>
  async (dispatch, _getState, client) => {
    await dispatch(createToggleFavoriteChannelAction(client, dispatch, _getState, contentIds, 'like', mode));
  };

export const removeFavoriteChannel = (contentId: string, mode: LiveContentMode = LIVE_CONTENT_MODES.all): EPGThunk =>
  async (dispatch, _getState, client) => {
    await dispatch(createToggleFavoriteChannelAction(client, dispatch, _getState, [contentId], 'remove-like', mode));
  };
