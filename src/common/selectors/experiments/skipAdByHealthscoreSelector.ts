...
type ExperimentValue = PS5_SKIP_AD_WITH_HEALTHSCORE_VALUE | COMCAST_SKIP_AD_WITH_HEALTHSCORE_VALUE | LGTV_SKIP_AD_WITH_HEALTHSCORE_VALUE | VIZIO_SKIP_AD_WITH_HEALTHSCORE_VALUE | XBOXONE_SKIP_AD_WITH_HEALTHSCORE_VALUE | WEB_SKIP_AD_WITH_HEALTHSCORE_VALUE | FIRETV_SKIP_AD_WITH_HEALTHSCORE_VALUE | AD_HEALTH_OPTIONS;
...
export const skipAdWithHealthScoreSelector = (state: StoreState): ExperimentValue => {
  if (!config) {
    return getExperiment(webottMajorPlatformsAdHealthscoreR1, { disableExposureLog: true }).get('skip_ad_factor') as ExperimentValue;
  }
