...
export const historySelector: Selector<HistoryState, HistoryState['contentIdMap']> = ({ contentIdMap }) => contentIdMap;

export const historyStateSelector = (state: StoreState) => state.history;

export const contentIdMapSelector = createSelector(
...
], (series, historyEpisode, formatMessage, episodeId) => {
  if (!series || !Array.isArray(series.seasons)) return '';

  const episodeContentId = episodeId || (historyEpisode
