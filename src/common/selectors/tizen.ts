...
import { isCWInPmrEnabledSelector } from './remoteConfig';
...
export const shouldReportContinueWatchingSelector = createSelector(
  isCWInPmrEnabledSelector,
  isLoggedInSelector,
  (state: StoreState) => state.consent.consents.some(({ key, value }) => key === CW_CONSENT_KEY && value === 'opted_in'),
  (state: StoreState) => state.history.loaded,
  (isSamsungCWInPmrEnabled, isLoggedIn, hasConsent, historyLoaded) => {
    const needConsent = isSamsungCWInPmrEnabled && !hasConsent;
