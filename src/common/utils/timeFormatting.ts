...
const messages = defineMessages({
  ...
  hoursAndMinutesShort: {
    description: 'hours and minutes (short)',
    defaultMessage: '{hours, plural, =0 {} other {#h }}{minutes, plural, =0 {} other {#m }}{remaining, select, true {left} other {}}',
  },
  sunday: {
...
type SecondsToHoursAndMinutesOptions = {
  remaining?: boolean;
  short?: boolean;
 ...
 */
export const secondsToHoursAndMinutes = (sec: number | undefined, { formatMessage, remaining = false, short = false }: SecondsToHoursAndMinutesOptions = {}): string => {
  if (!sec || sec === Infinity) {
    return '';
  }
  ...
  if (formatMessage && supportsPluralRules) {
    return formatMessage(short ? messages.hoursAndMinutesShort : messages.hoursAndMinutes, { hours, minutes, remaining }).trim();
  }

  return [
    hours ? `${hours}${short ? 'h' : ' hr'}` : '',
    minutes ? `${minutes}${short ? 'm' : ' min'}` : '',
    remaining ? 'left' : '',
  ].filter(Boolean).join(' ');
};
 ...
 * @returns {String}
 */
export const secondsToHMS = (sec?: number, onlyPaddingSeconds?: boolean): string => {
  if (!sec) {
    return '00:00:00';
  }
  ...
  const minutesSecondsStr = `${onlyPaddingSeconds ? minutes : getPaddingNumber(minutes)}:${getPaddingNumber(seconds)}`;

  let hoursStr = '';
  // if hours is 0, omit it
  if (hours) {
    hoursStr = `${onlyPaddingSeconds ? hours : getPaddingNumber(hours)}:`;
  }
