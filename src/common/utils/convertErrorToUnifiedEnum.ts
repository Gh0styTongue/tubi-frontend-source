...
export const QoSErrorCode = {
  ...
  PLAYER_BUFFER_SEEK_OVER_HOLE: 2019,
...
const getErrorCodeFromDetails = (error: ErrorEventData) => {
  switch (error.details) {
    ...
    case PLAYER_ERROR_DETAILS.PLAYER_BUFFER_SEEK_OVER_HOLE:
      return QoSErrorCode.PLAYER_BUFFER_SEEK_OVER_HOLE;
    default:
      break;
  }
...
const getErrorCodeFromErrorLogCode = (error: ErrorEventData) => {
  const errorLogCode = (MEDIA_ERROR_CODES[error.code as number] || error.code || error.type) ?? UNKNOWN_ERROR;
  switch (errorLogCode) {
    case ErrorType.SETUP_ERROR:
