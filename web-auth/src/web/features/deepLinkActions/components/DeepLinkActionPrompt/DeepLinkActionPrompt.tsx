...
const DeepLinkActionPrompt: FC<Props> = ({
  ...
  switch (action) {
    case DEEPLINK_ACTIONS.REMOVE_FROM_MY_LIST:
      dataLoaded = !isLoggedIn || queue.loaded;
      break;
  ...
  useEffect(() => {
    const hasData = action && currentContentId && currentTitle && dataLoaded;
  ...
  }, [action, contentType, currentContentId, currentDate, currentTitle, dataLoaded, didShowPrompt, dispatch, intl, isLoggedIn, isMajorEventFailsafe, location, majorEventFailsafeMessages, queue]);
