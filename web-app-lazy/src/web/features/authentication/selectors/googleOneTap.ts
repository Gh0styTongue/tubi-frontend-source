...
export const useFedCMForPromptSelector = createSelector(
  userAgentSelector,
  (userAgent) => {
    const { name: browserName, major: browserMajorVersion } = userAgent.browser;
    const browserVersion = browserMajorVersion ? Number(browserMajorVersion) : Number.NEGATIVE_INFINITY;
