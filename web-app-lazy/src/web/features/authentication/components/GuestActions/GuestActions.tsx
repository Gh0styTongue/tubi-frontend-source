...
const GuestActions: React.FC<Props> = (props) => {
  ...
  const redirectToEmailRegister = useCallback((e: any) => {
    e.preventDefault();
    attachRedirectCookie(generateRedirectUrl(redirect, false));
    tubiHistory.push(`${WEB_ROUTES.register}?redirect=${generateRedirectUrl(redirect)}`);
  }, [redirect]);

  const redirectToSignIn = useCallback((e: any) => {
    e.preventDefault();
    attachRedirectCookie(generateRedirectUrl(redirect, false));
    tubiHistory.push(`${WEB_ROUTES.signIn}?redirect=${generateRedirectUrl(redirect)}`);
  }, [redirect]);

  const attachRedirectCookieForGoogle = useCallback(() => {
