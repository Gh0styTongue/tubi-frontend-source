...
const messages = defineMessages({
  ...
  copyrightOriginalLink: {
    description: 'input form field label',
    defaultMessage: 'Links to original work',
  },
  contentToReport: {
    description: 'section name',
  ...
  nonOfAboveHint: {
    ...
    defaultMessage: 'Tubi only processes requests that are submitted by the owner of rights in a {issueType} or an authorized representative of a rights owner.',
  },
  success: {
    description:
...
export interface StubiosDMCAProps extends OwnProps, StateProps {
  dispatch: TubiThunkDispatch;
  intl: IntlShape;
}

export const StubiosDMCA: React.FunctionComponent<FormikProps<FormValues> & StubiosDMCAProps> = ({
  isMobile,
  ticketFieldIdMap,
  dynamicContentMap,
  handleSubmit,
  ...
  const issueOptions = useMemo(() => {
    return getFieldOptions(ticketFieldIdMap[STUBIOS_DMCA_ISSUE_TYPE_FIELD_ID], intl.formatMessage, dynamicContentMap);
  ...
  const ownerOptions = useMemo(() => {
    ...
    if (values.issueType === StubiosDMCAIssueTypeValue.trademarkInfringement) {
  ...
  const acknowledgementStatement = useMemo(() => {
    const fieldId = values.issueType === StubiosDMCAIssueTypeValue.trademarkInfringement
      ? STUBIOS_DMCA_TRADEMARK_ACKNOWLEDGEMENT_STATEMENT_FIELD_ID
      : STUBIOS_DMCA_COPYRIGHT_ACKNOWLEDGEMENT_STATEMENT_FIELD_ID;
    return [{
  ...
  useEffect(() => {
    if (values.reporterInformation
      ...
      && ((values.issueType === StubiosDMCAIssueTypeValue.copyrightInfringement && Object.values(StubiosDMCATrademarkReporterValue).includes(values.reporterInformation as StubiosDMCATrademarkReporterValue))
      || (values.issueType === StubiosDMCAIssueTypeValue.trademarkInfringement && Object.values(StubiosDMCACopyrightReporterValue).includes(values.reporterInformation as StubiosDMCACopyrightReporterValue)))
    ) {
  ...
  const issueType = values.issueType === StubiosDMCAIssueTypeValue.copyrightInfringement ? 'Copyright' : 'Trademark';
          ...
          ? <div>
            <div className={itemCls}>
              <h3 className={itemCls}>
                {intl.formatMessage(messages.contactInformationDescription, { issueType: issueType.toLowerCase() })}
              </h3>
              <h3 className={itemCls}>
            ...
            {
              values.issueType === StubiosDMCAIssueTypeValue.copyrightInfringement
                ? <div className={itemCls}>
                  <ComposedCheck
...
export const validate = (values: FormValues | null, { intl }: StubiosDMCAProps) => {
  const errors: StubiosDMCAProps['errors'] = {};
  ...
  if (values.issueType && values.reporterInformation && values.reporterInformation !== StubiosDMCACopyrightReporterValue.none) {
    ...
    switch (values.issueType) {
      case StubiosDMCAIssueTypeValue.copyrightInfringement:
        ...
        break;
      case StubiosDMCAIssueTypeValue.trademarkInfringement:
...
export const handleSubmit = (data: FormValues | null, formikBag: FormikBag<StubiosDMCAProps, FormValues>) => {
...
const supportWithFormik = withFormik({
  mapPropsToValues: (props: StubiosDMCAProps) =>
    ALLOWED_STUBIOS_DMCA_FIELDS.reduce(
...
const StubiosDMCAForm = connect(mapStateToProps)(injectIntl(supportWithFormik));
/**
 * StaticPage route expects functional component, so wrap the class here
 */
export default ({ location }: { location: Location }) => <StubiosDMCAForm location={location} />;
