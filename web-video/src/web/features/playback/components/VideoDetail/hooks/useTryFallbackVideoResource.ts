...
export const getFallbackPosition = ({ startPosition, playerRef }: GetFallbackPositionParams) => {
  ...
  const lastPosition = playerRef.current ? playerRef.current.getPosition() : 0;
...
export const useTryFallbackVideoResource = ({
  ...
  const tryFallbackVideoResource = useCallback((error: ErrorEventData) => {
    const resumePosition = getFallbackPosition({ startPosition: startPositionRef.current, playerRef });
