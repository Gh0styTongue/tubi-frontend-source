...
import { loadEpisodesInSeries, loadRelatedContents } from 'common/actions/video';
import { EPISODE_PAGINATION_PAGE_SIZE, RELATED_CONTENTS_LIMIT_LEGACY } from 'common/constants/constants';
...
import { getPath, getCanonicalLink, getCanonicalMetaByLink, getSEOPageTitle } from 'web/features/seo/utils/seo';

interface ExperimentProps {
  webIosPlayback: ReturnType<typeof WebIosPlayback>;
  webAndroidDisablePlayback: ReturnType<typeof WebAndroidDisablePlayback>;
...
class Series extends Component<Props> {
  ...
  componentDidMount(): void {
    if (this.props.userAgent && isMobileWebkit(this.props.userAgent)) {
      this.props.webIosPlayback.logExposure();
    }
    if (this.props.userAgent && isChromeOnAndroidMobile(this.props.userAgent)) {
      this.props.webAndroidDisablePlayback.logExposure();
    }
    this.props.webRepositionVideoResource.logExposure();
  }

  getMeta() {
    const { intl, series, params, deviceId } = this.props;
    ...
    return {
      title: getSEOPageTitle({ title, limit: Number.POSITIVE_INFINITY }),
...
export function fetchData({ dispatch, params }: FetchDataParams<{ id: string }>) {
  ...
  return dispatch(loadSeriesAction).catch((error: UserFacingError) => {
...
export function fetchDataDeferred({ dispatch, getState, params }: FetchDataParams<{ id: string }>) {
  ...
  promises.push(
    dispatch(loadRelatedContents(contentId, RELATED_CONTENTS_LIMIT_LEGACY)).catch((error: any) => {
      const loggerType = getLogLevel(error.errType);
...
export const mapStateToProps = (state: StoreState, ownProps: OwnProps): StateProps => {
  ...
  const { isMobile, userAgent, isKidsModeEnabled } = ui;
  ...
  const isInMobileWhitelist = isMobile && mobilePlaybackSupported({ userAgent, enableMobileWebIosPlayback, enableWebAndroidPlayback });
...
export default withExperiment(connect(mapStateToProps)(IntlSeries), {
  webIosPlayback: WebIosPlayback,
  webAndroidDisablePlayback: WebAndroidDisablePlayback,
  webRepositionVideoResource: WebRepositionVideoResource,
});
