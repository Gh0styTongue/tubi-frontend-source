...
const messages = defineMessages({
  label: {
    description: 'default label for google button',
...
export class GoogleButton extends React.PureComponent<AllProps, State> {
  ...
  handleCodeResponse = (response: any) => {
    const { dispatch, isMajorEventActive } = this.props;
    const { code } = response;
  ...
  render() {
    const { label, className, icon, intl } = this.props;
    const Icon = icon;
    /* istanbul ignore next */
    const buttonLabel = label || intl.formatMessage(messages.label);
...
export const mapStateToProps = (state: StoreState): StateProps => {
  return {
    isMajorEventActive: isMajorEventActiveSelector(state),
  };
};

export default injectIntl(connect(mapStateToProps)(GoogleButton)) as React.FC<Props>;
