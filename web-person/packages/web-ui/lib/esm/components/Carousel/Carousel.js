...
import React, { useEffect, useRef, useState, memo, useCallback, useLayoutEffect, useReducer } from 'react';
...
function Carousel(_a) {
    ...
    // Hack to avoid warnings on server rendering. This use of layout effect
    // should be safe since the ref is already initialized above on the server
    ...
    var debouncedResizeHandler = useCallback(debounce(resizeHandler, 250), []);
    useEffect(function () {
    ...
    var hasNext = index + colsPerPage < colCount;
    var getKey = function (item) {
        if (typeof item === 'string') {
            return item;
        }
        if (item === null || item === void 0 ? void 0 : item.id) {
            return String(item === null || item === void 0 ? void 0 : item.id);
        }
        return undefined;
    };
    ...
    return (React.createElement(CarouselShell, { adjustPrevNextForContentTile: adjustPrevNextForContentTile, className: className, style: style, hasNext: hasNext, hasPrevious: hasPrevious, onNext: next, onPrevious: previous, showItemOverflow: showItemOverflow },
        React.createElement("div", { className: "web-carousel__container" },
            React.createElement(Grid.Container, { ref: containerRef, className: classNames(bem.block, (_b = {},
