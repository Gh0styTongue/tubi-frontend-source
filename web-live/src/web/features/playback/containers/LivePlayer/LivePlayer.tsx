...
import { FREEZED_EMPTY_OBJECT, HOME_DATA_SCOPE, CONTENT_MODES, DEFAULT_VIDEO_CONTENT_DISASTER } from 'common/constants/constants';
...
import * as purpleCarpetSelectors from 'common/features/purpleCarpet/selector';
...
export const LivePlayer: TubiContainerFC<Props, RouteParams> = ({
  ...
  const isPurpleCarpetContent = useAppSelector((state) => purpleCarpetSelectors.isPurpleCarpetContentSelector(state, contentId));
  const shouldLockPurpleCarpet = useAppSelector(purpleCarpetSelectors.shouldLockPurpleCarpetSelector);
  const isDisasterMode = useAppSelector(purpleCarpetSelectors.shouldEnableDisasterMode);
  const notStarted = (video && 'air_datetime' in video && video.air_datetime) ? new Date(video.air_datetime).getTime() > Date.now() : false;
  const { action } = useLocation();

  useLayoutEffect(() => {
    if (!isPurpleCarpetContent || isDisasterMode) return;
    // redirect to the details page:
    // when the guest user landing to the purple carpet playback page directly
    // when the mobile user landing to the purple carpet playback page
    // when the deeplink to the playback page and the status of purple carpet content is not started
    if (shouldLockPurpleCarpet || isMobile || (action === 'POP' && notStarted)) {
  ...
  }, [shouldLockPurpleCarpet, isPurpleCarpetContent, video, isMobile, notStarted, action, isDisasterMode]);

  if (isDisasterMode) {
    video = DEFAULT_VIDEO_CONTENT_DISASTER;
  }
  ...
  return (
    <React.Fragment>
      ...
      <div className={styles.root}>
        ...
        {isPurpleCarpetContent || isDisasterMode
        ...
        {!isMobile && (
          <React.Fragment>
            {isPurpleCarpetContent || isDisasterMode ? null : <ChannelDetails channel={video} />}
            <Footer purpleCarpet={isPurpleCarpetContent} useRefreshStyle contentId={contentId} />
          </React.Fragment>
        )}
      </div>

      <VideoObjectSchema video={isDisasterMode ? DEFAULT_VIDEO_CONTENT_DISASTER : video} />
    </React.Fragment>
  );
};
...
LivePlayer.fetchData = async ({ getState, dispatch, params }) => {
  const { id } = params;
  const isDisasterMode = purpleCarpetSelectors.shouldEnableDisasterMode(getState());
  dispatch(setLiveVideoPlayer(PlayerDisplayMode.DEFAULT));

  try {
    let video = liveVideoSelector(getState(), id);
    if (!video && !isDisasterMode) {
