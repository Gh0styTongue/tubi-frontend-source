...
export const usePurpleCarpetCountdown = (startTime?: number, id?: string, shouldIgnorePcStatus?: boolean) => {
  ...
  useEffect(() => {
    ...
    if (status === PurpleCarpetStatus.NotAvailable && !shouldIgnorePcStatus) {
  ...
  }, [startTime, status, id, dispatch, resetTimeDiff, shouldIgnorePcStatus]);
