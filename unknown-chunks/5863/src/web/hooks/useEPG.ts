...
export const useWebEPGData = (mode: LiveContentMode, timelineStart: number, timelineEnd: number) => {
  ...
  const onChannelEnteredView = useCallback(
    (id: string, index: number) => {
      if (byId[id] && isCacheValid(byId[id].ttl)) {
        return;
      }
      ...
      if (contentIdsToFetchRef.current.size > contentIdsToFetch.length || Object.keys(byId).length < contentIdsToFetch.length) {
        throttledSetContentIdsToFetch(Array.from(contentIdsToFetchRef.current));
      }
    },
    [byId, contentIds, contentIdsToFetch, throttledSetContentIdsToFetch]
  );
  ...
  return {
    ...
    contentIdsToFetch, // return contentIdsToFetch for test
  };
};
