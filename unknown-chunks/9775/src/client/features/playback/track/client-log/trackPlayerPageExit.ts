...
export function trackPlayerPageExit({
  ...
  const ps4HardwareId = __OTTPLATFORM__ === 'PS4' ? (systemApi as PS4SystemApi).getHardwareId() : undefined;
  ...
  const playerAnalyticsData = {
    ...
    current_position: convertUnitWithMathRound(position, 1000), // millisecond
  };
  trackLogging({
    ...
    message: {
      ...
      bufferedArray: player?.getBufferedVideoRange?.(),
      audioBufferedArray: player?.getBufferedAudioRange?.(),
      bufferLength,
      utmSource,
      isAPPLaunchedFromDeepLink,
      hdmiConnectionStatus,
  ...
  if (__ENABLE_PLAYER_EXIT_TO_DATADOG__) {
    ...
    let playerExitMetricsParam: PlayerExitMetricsParam = {
      hasError: !!lastError,
      stage: reportStageMap[stage],
      totalViewTime,
    };
    if (__OTTPLATFORM__ === 'PS4') {
      playerExitMetricsParam = {
        ...playerExitMetricsParam,
        [CUSTOM_TAGS.WEBMAF]: (systemApi as PS4SystemApi).getWebMAFVersion?.() ?? '2',
      };
    }
