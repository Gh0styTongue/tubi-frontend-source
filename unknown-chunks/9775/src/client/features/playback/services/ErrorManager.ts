...
import { getConfig as ottLgtvSrcNotSupportedErrorRecoveryGetConfig } from 'common/experiments/config/ottLgtvSrcNotSupportedErrorRecovery';
...
/**
 * We still use the WebMAFPlayerWrapper for WebMAF 2.9.1. The ErrorManager can
 * be instantiated with a WebMAFPlayerWrapper. This function determines whether we have
 * Player or WebMAFPlayerWrapper.
 */
function isPlayer(player: undefined | Player | WebMAFPlayerWrapper): player is Player {
  return player !== undefined && !(player as WebMAFPlayerWrapper)._handleTTY;
}

export class ErrorManager extends BasePlayerManager<Player | WebMAFPlayerWrapper> {
  ...
  receivedError = (error: ErrorEventData) => {
    ...
    trackVODError(error, {
      ...
      cdn: isPlayer(player) ? player.getCDN() : undefined,
    });
  ...
  private handleOTTError(player: Player | WebMAFPlayerWrapper, error: ErrorEventData) {
    const playerName = player.playerName;
    if (playerName === PlayerName.VOD && isPlayer(player)) {
  ...
  private shouldRecoverSrcNotSupportedError() {
    const experiment = this.youboraExperimentMap[ottLgtvSrcNotSupportedErrorRecoveryGetConfig().experimentName];
    experiment?.logExposure();
    const result = !!experiment?.getValue();
  ...
  private handleVideoSessionCollect(player: Player | WebMAFPlayerWrapper, error: ErrorEventData) {
    const playerName = player.playerName;
