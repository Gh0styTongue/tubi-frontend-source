...
const triggerMockError = (key: string) => {
  try {
    if (mockHLSError[key]) {
      window.Tubi.player.adapter.extension.hls.trigger('hlsError', mockHLSError[key]);
    } else if (key === 'playInterruptedError') {
      const { videoElement } = window.Tubi.player.adapter;
      ...
      window.Tubi.player.adapter.emit('tb_error', mockPlayerError[key]);
    } else {
