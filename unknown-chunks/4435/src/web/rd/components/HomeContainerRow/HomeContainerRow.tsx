...
import { isHoverOnTilePreviewSelector, shouldHideMetadataSelector } from 'common/selectors/ui';
...
const HomeContainerRow = forwardRef<HTMLElement, Props>(({
  ...
  // The Featured Row should be portrait when purple carpet row displaying
  const showPurpleCarpetRow = useAppSelector(state => shouldShowPurpleCarpetOnHomeScreenSelector(state, { pathname }));
  const tileOrientation: TileOrientation =
    containerType === CONTAINER_TYPES.LINEAR || (isFeaturedContainer && !showPurpleCarpetRow) ? 'landscape' : 'portrait';
  ...
  const hideMetadata = useAppSelector(state => shouldHideMetadataSelector(state, pathname));
  ...
  const renderCarouselItem = useCallback(
    (
      ...
      if (containerType === CONTAINER_TYPES.LINEAR) {
        return (
          <LinearTile
            key={id}
            id={id}
    ...
    [containerId, containerType, handleTileNavigation, isMyStuffPage, showLinearProgramsInRows, tileOrientation, personalizationId, hideMetadata, containerPosition]
  );
  ...
  const rowHeaderCls = classNames(styles.rowHeader, { [styles.videoPreviewEnabled]: isVideoPreviewEnabled });
  ...
  if (isMyStuffPageEmptyRow) {
    ...
    if (isFeaturedContainer && !showPurpleCarpetRow) {
