...
import { isPurpleCarpetSupportedSelector, isWebPurpleCarpetPreviewEnabledSelector } from 'common/features/purpleCarpet/selector';
...
export const PurpleCarpetDetails: FC<{ id: string; type: 'row' | 'fullscreen', loading?: boolean }> = memo(
  ({ id, type = 'row', loading }) => {
    ...
    const bg = hashImageDomain(
      isMobile
        ? images?.hack_feat_hero_sl?.[0] ?? images?.hero_feature?.[0] ?? backgroundImage
        : backgroundImage
    );
      ...
      >
        ...
        <div className={styles.contentWrapper}>
          <div className={styles.content}>
            ...
            <div className={styles.title}>{titleArt ? <img alt={title} src={titleArt} fetchPriority="high" /> : title}</div>
            { tags?.length ?
