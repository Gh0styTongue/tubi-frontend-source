...
import type WebMAFPlayerAdapter from './liveAdapters/webmaf';
...
let WebMAFAdapter: undefined | typeof WebMAFPlayerAdapter;

// istanbul ignore next
if (__TESTING__ || (__OTTPLATFORM__ === 'PS4' && !(systemApi as PS4SystemApi).isWebMAF3())) {
  WebMAFAdapter = require('./liveAdapters/webmaf').default;
}
...
export class LivePlayerWrapper extends PlayerEventEmitter<LivePlayerListeners | LiveAdPlayerWithApolloListeners> {
  ...
  async setup() {
    ...
    if (__OTTPLATFORM__ === 'PS4' && typeof WebMAFAdapter !== 'undefined') {
      this.loadWebMAF({
        mediaUrl,
        videoElement,
      });
  ...
  private loadWebMAF = ({
    mediaUrl,
    videoElement,
  }: Pick<LiveAdapterConfig, 'mediaUrl' | 'videoElement'>) => {
    const webMAFAdapter = new WebMAFAdapter!({
      mediaUrl,
      videoElement,
      debugLevel: FeatureSwitchManager.isDefault(['Logging', 'Player']) ? PLAYER_LOG_LEVEL.DISABLE : FeatureSwitchManager.get(['Logging', 'Player']) as PLAYER_LOG_LEVEL,
    });
    this.adapter = webMAFAdapter;
  };
