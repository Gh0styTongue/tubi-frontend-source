...
import { utf8ArrayToStr } from 'client/utils/textDecoder';
...
export default class ApolloLiveAdClient extends PlayerEventEmitter<LiveAdPlayerWithApolloListeners> {
  ...
  private parseAdInfoFromTagList = (tagsArray: string[][]): string | undefined => {
    ...
    for (const key in pixels) {
      ...
      if (pixels.hasOwnProperty(key)) {
        if (!pixelMap.trackings[key]) {
          pixelMap.trackings[key] = [];
        }
        // We should prevent pixel url be saved twice
        pixelMap.trackings[key] = Array.from(new Set(pixelMap.trackings[key].concat(pixels[key])));
      }
    }
  ...
  private parseSamples = (samples: MetadataSample[], adId: string): void => {
    for (const current of samples) {
      // The id3 tag sample data is formatted as ArrayBuffer
      // pts: The position we should send corresponding TPOS
      // TPOS: The play progress of current ad. We use it to find tracking pixels urls
      // For more detail. Refer to https://www.notion.so/tubi/Timed-Metadata-in-Gaia-Ad-Videos-6f7fa086942f41e082de858c3a38648d
      let p = 10;
      const frames: { TPOS?: string, TDUR?: string } = {};
      while (p < current.data.length) {
        const key = utf8ArrayToStr(current.data.slice(p, p + 4));
        const length = new DataView(current.data.slice(p + 4, p + 8).buffer, 0).getUint32(0);
        const payload = utf8ArrayToStr(current.data.slice(p + 10, p + 10 + length));
        frames[key] = payload;
        p = p + 10 + length;
      }

      if (frames.TPOS && current.pts) {
        const position = current.pts;
        const progress = parseInt(frames.TPOS, 10) as QuartilePercent;
