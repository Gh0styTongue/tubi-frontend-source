...
import { getConfig as ottFireTVErrorRecoveryResetRetryRecordGetConfig } from 'common/experiments/config/ottFireTVErrorRecoveryResetRetryRecord';
...
const SRC_NOT_SUPPORTED_ERR_HOLDOUT_PLATFORMS = ['LGTV'];
...
export class ErrorManager extends BasePlayerManager<Player> {
  ...
  reloadSuccess = () => {
    if (this.reloadCount > 0) {
      const experiment = this.youboraExperimentMap[ottFireTVErrorRecoveryResetRetryRecordGetConfig().experimentName];
      experiment?.logExposure();
      const result = !!experiment?.getValue();
      VODPlaybackSession.getInstance().setFeatureInfo('resetRetryRecord', result);
      if (result) {
        this.reloadCount = 0;
      }
    }
  ...
  private shouldRecoverSrcNotSupportedError() {
    ...
    experiment?.logExposure();
    const result = !SRC_NOT_SUPPORTED_ERR_HOLDOUT_PLATFORMS.includes(__OTTPLATFORM__) || !!experiment?.getValue();
