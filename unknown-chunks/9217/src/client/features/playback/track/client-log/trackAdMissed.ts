...
// Allows us to make changes to the BWW experiment without having
// to version it up, breaking down results by log version
export const BWW_VERSION = '7';
...
export function trackAdMissed(data: TrackAdMissedMessage) {
  const { response, reason, currentBreak, targetBreak, detail, adSequence = 0, metrics, position, bwwActive, controlsActive } = data;
