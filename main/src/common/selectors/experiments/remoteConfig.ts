...
export const livePerformanceMetricEnabledSelector = createSelector(
  ...
  (deviceId, hasPerformanceApi, featureSwitch) => hasPerformanceApi && (sampleString(deviceId, rate.performance.live, 'livePerformanceMetric') || featureSwitch),
);
...
export const vodPerformanceMetricEnabledSelector = createSelector(
  ...
  () => hasNecessaryPerformanceApi(),
  () => FeatureSwitchManager.isEnabled(['Player', 'Performance']),
  (deviceId, hasPerformanceApi, featureSwitch) => hasPerformanceApi && (sampleString(deviceId, rate.performance.vod, 'vodPerformanceMetric') || featureSwitch),
);
...
export const previewPerformanceMetricEnabledSelector = createSelector(
  ...
  () => hasNecessaryPerformanceApi(),
  () => FeatureSwitchManager.isEnabled(['Player', 'Performance']),
  (deviceId, hasPerformanceApi, featureSwitch) => hasPerformanceApi && (sampleString(deviceId, rate.performance.preview, 'previewPerformanceMetric') || featureSwitch),
);

export const uiNavigationPerformanceMetricEnabledSelector = createSelector(
  (state: StoreState) => String(state.auth.deviceId),
  () => hasNecessaryPerformanceApi(),
  (deviceId, hasPerformanceApi) => hasPerformanceApi && (sampleString(deviceId, 0.1, 'uiNavigationPerformanceMetric')),
);
