...
import { SB_CONTENT_IDS } from 'web/features/purpleCarpet/containers/SuperBowl/sb-constants';
...
export function getDeepLinkForVideo(content: Video | ChannelEPGInfo, deviceId?: string, options: { stopTracking?: boolean } = {}): string {
  ...
  // For production, use the Sealion branch.io link for SB_CONTENT_IDS
  // For alpha/staging/dev, use the Sealion branch.io link for contents with player_type=fox to test during rehearsal
  if (
    SB_CONTENT_IDS.includes(content.id) ||
    ((!__PRODUCTION__ || __IS_ALPHA_ENV__) && 'player_type' in content && content.player_type === 'fox')
  ) {
