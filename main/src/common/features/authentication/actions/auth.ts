...
export const redirectContinueAsGuestOnOTT = (): TubiThunkAction => {
  return (dispatch: TubiThunkDispatch, getState: () => StoreState) => {
    ...
    const currentPathname = tubiHistory.getCurrentLocation().pathname;
    tubiHistory.goBack();

    // A defensive fix to prevent the user from being stuck due to 'goBack' not working.
    // Details: https://github.com/adRise/www/pull/23170/files#r1921590508
    if (currentPathname === tubiHistory.getCurrentLocation().pathname) {
