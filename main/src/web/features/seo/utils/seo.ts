...
export const toString = (detail: string | ReactNode | (string | ReactNode)[]): string => {
  if (Array.isArray(detail)) {
    return detail.map(toString).join('');
  }

  if (React.isValidElement(detail)) {
    return toString(detail.props.children);
  }
...
export const formatFaqItemsForSeo = (items: FaqItem[]): FaqPageItem[] => {
  return items.map(({ title, detail }) => ({ question: title, answer: toString(detail) }));
};
