...
const TopNav: FC<Props> = ({ pathname }) => {
  ...
  const isDisasterMode = useAppSelector(shouldEnableDisasterMode);
  ...
  const KidsModeButton = shouldShowSuperbowl ?
    (<button id="kidsModeButton" className={classnames(navItemClass, styles.kidsAndEspanolLink)} onClick={handleEnterKidsMode}>
  ...
  const EspanolButton = shouldShowSuperbowl ?
    (<button id="espanolModeButton" className={classnames(navItemClass, styles.kidsAndEspanolLink)} onClick={toggleEspanolMode} data-test-id="top-nav-español">
    ...
    () => {
      return (
        ...
        >
          ...
          { shouldShowSuperbowl ? (
            <Link
              to={WEB_ROUTES.superBowl}
              onClick={hideMobileMenu}
              className={classnames(navItemClass, { [styles.active]: matchesRoute(WEB_ROUTES.superBowl, pathname) })}
            >
              {intl.formatMessage(topNavMessages.superBowl)}
            </Link>
          ) : null
          }
          ...
          {isEspanolModeEnabled ? (
            <button id="espanolModeButton" className={classnames(navItemClass, styles.exitEspanol)} onClick={toggleEspanolMode}>
              <Exit />
